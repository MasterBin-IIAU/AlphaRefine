...
def draw_success_precision(success_ret, name, videos, attr, precision_ret=None,
    ...
    ax.legend(loc='lower left', labelspacing=0.2,fontsize='x-small')
    ...
    # plt.show()
    plt.savefig('Success.pdf',format='pdf')
    ...
    if precision_ret:
        ...
        for idx, (tracker_name, pre) in \
            ...
            plt.plot(thresholds, np.mean(value, axis=0),
                    color=COLOR[idx], linestyle=LINE_STYLE[idx],label=label, linewidth=2)
        ax.legend(loc='lower right', labelspacing=0.2,fontsize='x-small')
        ax.autoscale(enable=True, axis='both', tight=True)
        xmin, xmax, ymin, ymax = plt.axis()
        ax.autoscale(enable=False)
        ymax += 0.03
    ...
    if norm_precision_ret:
        ...
        ax.set_aspect((xmax - xmin)/(ymax-ymin))
        # plt.show()
        plt.savefig('Norm Precision.pdf',format='pdf')