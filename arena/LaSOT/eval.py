...
parser.add_argument('--tracker_path', '-p', type=str, default=save_dir,
...
def main():
    ...
    if 'OTB' in args.dataset:
        ...
        if False:  # which videos are benefited from AlphaRefine
            import numpy as np
            auc_video = [np.mean(list(i.values()), -1) for i in success_ret.values()]
            names = list(list(success_ret.values())[0].keys())
            video_index = (auc_video[0] - auc_video[1]).argsort().tolist()
            names = [names[i] for i in video_index]
            print(names)
        ...
        if False:  # which videos are benefited from AlphaRefine
            ...
            auc_video = [np.mean(list(i.values()), -1) for i in success_ret.values()]
            names = list(list(success_ret.values())[0].keys())
            video_index = (auc_video[0] - auc_video[3]).argsort().tolist()
            names = [names[i] for i in video_index]
            print(names)
