...
from external.pysot.pysot.utils.bbox import cxy_wh_2_rect
...
class SiamMaskTracker(SiamRPNTracker):
    ...
    def _mask_post_processing(self, mask, VOT=False):
        ...
        if len(contours) != 0 and np.max(cnt_area) > 100:
            ...
            if VOT:
                prbox = np.array(cv2.boxPoints(cv2.minAreaRect(polygon))) # [[x1,y1],[x2,y2],[x3,y3],[x4,y4]]
            else:
                prbox = np.array(cv2.boundingRect(polygon)) # [x1,y1,w,h]
            rbox_in_img = prbox
        else:  # empty mask
            if VOT:
                ...
                rbox_in_img = np.array([[location[0], location[1]],
                                    ...
                                    [location[0], location[1] + location[3]]]) # 8 points
            else:
                rbox_in_img = cxy_wh_2_rect(self.center_pos, self.size) # 4 points (x1,y1,w,h)
        return rbox_in_img

    def track(self, img, VOT=True):
        """
        args:
        ...
        delta_x, delta_y = pos[2], pos[1] # 在下采样后搜索区域上的位置(17x17)

        mask = self.model.mask_refine((delta_y, delta_x)).sigmoid().squeeze()
        out_size = cfg.TRACK.MASK_OUTPUT_SIZE
        mask = mask.view(out_size, out_size).cpu().data.numpy() # (127,127)
        '''如何把mask映射回原图,可以参考下下面这段代码'''
        '''crop_box是裁剪下的搜索区域在原图上的(x1,y1,w,h)'''
        s = crop_box[2] / cfg.TRACK.INSTANCE_SIZE # 真实的搜索区域尺寸/255
        base_size = cfg.TRACK.BASE_SIZE # 8
        stride = cfg.ANCHOR.STRIDE # 8
        ...
        '''把127x127的mask映射成(im_w,im_h)这么大'''
        mask_in_img = self._crop_back(mask, back_box, (im_w, im_h))
        polygon = self._mask_post_processing(mask_in_img,VOT=VOT) # ndarray
        if VOT:
            '''return 8 points'''
